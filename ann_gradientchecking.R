...
# Creating cost function to evaluate parameter estimates and improve our neural network
cost <- function(y, y_hat) {
  cost <- 0.5*sum((y - y_hat)^2)
  return(cost)
}

# Creating a placeholder for y-hat values
...
# Numeric gradient check to ensure partial derivatives are correct
# Setting size of perturbation
e <- 1e-4  
...
# Calculate the numeric gradient for each value in the W1 matrix
for(i in 1:elements){  
  ...
  A_2 <- sigmoid(Z_2 + t(B1 %*% rep(1,n)))
  Z_3 <- A_2 %*% W_2
  init_y_hat <- sigmoid(Z_3 + t(B2 %*% rep(1,n)))
  currentcost <- cost(y_train,init_y_hat)
  ...
  numgrad_w_1[i] <- (cost(y_train,y_hat) - currentcost)/e 
}
